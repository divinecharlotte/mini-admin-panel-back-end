...
// async getCountsLast7Days() {
//   // in UsersService.getCountsLast7Days()
// const raw = await this.repo
// .createQueryBuilder('u')
// .select("strftime('%Y-%m-%d', u.createdAt, 'localtime')", 'day')
// .addSelect('COUNT(*)', 'count')
// .where("u.createdAt >= datetime('now','start of day','-6 days')")
// .andWhere("u.createdAt < datetime('now','start of day','+1 day')")
// .groupBy('day')
...
// const d = new Date(start);
// d.setDate(start.getDate() + i);
// counts.set(toYmd(d), 0);
// }
// for (const row of raw) counts.set(row.day, Number(row.count) || 0);
